// ===== VERSION MARKER: V71-HOMEPAGE-SUMMARY-TRIAL-INFO =====
export default {
  async fetch(request, env, ctx) {
    const url = new URL(request.url);
    const { lang, remainder } = parseLang(url.pathname);

    if (remainder === "/" ) return await renderHome(env, lang, false, request);
    if (remainder === "/news") return await renderHome(env, lang, true, request);
...
async function renderHome(env, lang, isNewsPage, request) {
  ...
  for (const cat of categories) {
    ...
    const summaries = await mapWithConcurrency(results, 1, (row) =>
      getOrExpandSummary(env, row, request)
    );
...
async function checkTrialAccess(request, env) {
  ...
  const startedAt = new Date(user.trial_started_at + "Z");
